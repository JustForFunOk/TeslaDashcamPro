...
* 采集双闪信号时左转右转信号状态，双闪里面只有触发原因没有精确的亮灭控制

* 优化信号灯闪烁逻辑，不根据信号高低显示，触发之后根据播放的时钟闪烁，亮灭持续时间待确定，但是这样处理代码逻辑更复杂，待确定

* 看了数据 10s钟左转向灯闪烁了14次，平均0.7s闪烁一次 为了方便处理就按0.8s，亮0.4s，灭0.4s
